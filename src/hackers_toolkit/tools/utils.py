...
def register_tool(tool_class: Type["BaseTool"]) -> Type["BaseTool"]:
    assert hasattr(
        tool_class, "name"
    ), f"Tool class {tool_class.__name__} has no 'name' attribute."
    assert hasattr(
        tool_class, "categories"
    ), f"Tool class {tool_class.__name__} has not 'categories' attribute."
    ...
    def build_ui(self) -> Widget:
        return Box(
            children=[
                ...
                Label("Please select a tool on the left panel."),
            ],
